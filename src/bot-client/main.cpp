...
void print_header() {
    std::cout << 
        ...
        "   __        __          ___          __ \n"
        "  / /  ___  / /_________/ (_)__ ___  / /_\n"
        " / _ \\/ _ \\/ __/___/ __/ / / -_) _ \\/ __/\\"
        "/_.__/\\___/\\__/    \\__/_/_/\\__/_//_/\\__/ \n"
...
int main(int argc, char *argv[]) {
    ...
    try {
        ...
        if(argc < 2 || vm.count("help") || vm.count("bot") != 1) {
            std::cout << options.get_help_message() << std::endl;
            return 1;
        }

        BotClient botClient(vm["host"].as<std::string>(), vm["port"].as<unsigned short>(), vm["bot"].as<std::string>());
        botClient.run();
    } catch(po::error &ex) {
